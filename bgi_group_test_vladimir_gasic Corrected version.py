...
nbrs = NearestNeighbors(n_neighbors=6).fit(adata.to_df())
neigh_dist, neigh_ind = nbrs.kneighbors(adata.to_df())
sort_neigh_dist = np.sort(neigh_dist, axis=0)

k_dist = sort_neigh_dist[:, 4]
plt.plot(k_dist)
plt.axhline(y=7, linewidth=1, linestyle='dashed', color='k')
plt.ylabel("k-NN distance")
plt.xlabel("Sorted observations (4th NN)")
plt.figure(figsize=(200, 200))
plt.show()

clusters=DBSCAN(eps=7, min_samples=6).fit(adata.to_df())
clusters.labels_

len(clusters.labels_)

cl=list(set(clusters.labels_))
print(cl)
...
df1_scaled=sca.fit_transform(df1)
...
df2_principal = pca_spec.fit_transform(df2_final)
df2_principal = pd.DataFrame(df2_principal)
df2_principal.columns=["PC1", "PC2"]
df2_principal.head()

len(df2_principal)

plt.scatter(df2_principal.PC1, df2_principal.PC2)
...
df2_final_ex.x=df2_final_ex.x.apply(lambda x: x*10)

df2_final_ex.y=df2_final_ex.y.apply(lambda x: x*10)

df2_final_ex["Cluster Index Final"]=df2_final_ex["Cluster Index 1"]+df2_final_ex["Cluster Index 2"]
df2_final_ex.head()
